...
namespace PGII_TU_MOSQUITO_CLARES_CORTEZ_AROCUTIPA.Ejercicios
{
    public partial class Ejercicio_18 : Form
    {
        ...
        private void btn_Generar_Matricula_Click(object sender, EventArgs e)
        {
            ...
            foreach (string elemento in lst_Cursos_Matricula.Items) // 👈 recorre TODOS los ítems
            {
                int inicio = elemento.IndexOf('[') + 1;
                int fin = elemento.IndexOf(']');
                if (inicio > 0 && fin > inicio)
                {
                    string numeroStr = elemento.Substring(inicio, fin - inicio);
                    if (int.TryParse(numeroStr, out int creditos))
                    {
                        totalCreditos += creditos;
                        string nombreCurso = elemento.Substring(0, inicio - 1).Trim();

                        dgv_Matricula.Rows.Add(ObjEstudiante.codigo,nombreCurso, creditos);
                    }
                }
            }
        }
